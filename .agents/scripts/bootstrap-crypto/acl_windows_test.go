...
func TestApplySecureACL(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "acltest-*")
	if err != nil {
...
func TestExtractExplicitDACL(t *testing.T) {
	...
	if control&windows.SE_DACL_PRESENT == 0 {
		t.Errorf("SE_DACL_PRESENT is not set")
	}
	...
	if dacl == nil {
		t.Errorf("DACL is nil")
	}

	if defaulted {
		t.Errorf("defaulted is true, expected false")
	}

	extracted, err := extractExplicitDACL(sd)
	if err != nil {
		t.Fatalf("extractExplicitDACL failed: %v", err)
	}

	if extracted == nil {
		t.Errorf("extractExplicitDACL returned nil ACL")
	}
}
